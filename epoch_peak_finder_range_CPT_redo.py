...
selected_channels = ["FCz", "Cz", "Pz", "Fz"]
#
#######################################
...
def set_peak_options(file_name, min, max, peak_num):
    ...
    if (callback_id == "min-button") or (callback_id == "max-button"):
        if file_name[-2:] == "_2" or file_name[-2:] == "_3":
            ...
            return (
                peak_dropdown_options_2_3,
                peak_dropdown_options_2_3[(peak_index + 1) % 3],
            )
...
def update_graph(y_min, channel, file_name, view_peak, table_data, peak_box):
    ...
    if view_peak == ["View Peaks"]:
        ...
        # fig.add_trace(
        #     go.Scatter(
        #         x=peak_df_graph.loc[peak_df_graph["Channel"] == "FCz"]["x"],
        ...
        # fig.add_trace(
        #     go.Scatter(
        #         x=peak_df_graph.loc[peak_df_graph["Channel"] == "Cz"]["x"],
        ...
        # fig.add_trace(
        #     go.Scatter(
        #         x=peak_df_graph.loc[peak_df_graph["Channel"] == "Fz"]["x"],
        ...
        # fig.add_trace(
        #     go.Scatter(
        #         x=peak_df_graph.loc[peak_df_graph["Channel"] == "Pz"]["x"],
...
def find_peaks(
    ...
    if callback_id == "auto-button":
        ...
        if subject_file[-2:] == "_2" or subject_file[-2:] == "_3":
            auto_peak_1 = temp_df.loc[
                (temp_df["x"] >= con_2_3_ranges[0][0])
                & (temp_df["x"] <= con_2_3_ranges[0][1])
            ]
            auto_peak_1 = auto_peak_1.loc[
                (auto_peak_1.groupby(["Channel"])["y"].idxmax())
            ]
            auto_peak_1["Range"] = str(con_2_3_ranges[0])
            auto_peak_1["Peak"] = "P1"

            auto_peak_2 = temp_df.loc[
                (temp_df["x"] >= con_2_3_ranges[1][0])
                & (temp_df["x"] <= con_2_3_ranges[1][1])
            ]
            auto_peak_2 = auto_peak_2.loc[
                (auto_peak_2.groupby(["Channel"])["y"].idxmin())
            ]
            auto_peak_2["Range"] = str(con_2_3_ranges[1])
            auto_peak_2["Peak"] = "N2"

            auto_peak_3 = temp_df.loc[
                (temp_df["x"] >= con_2_3_ranges[2][0])
                & (temp_df["x"] <= con_2_3_ranges[2][1])
            ]
            auto_peak_3 = auto_peak_3.loc[
                (auto_peak_3.groupby(["Channel"])["y"].idxmax())
            ]
            auto_peak_3["Range"] = str(con_2_3_ranges[2])
            auto_peak_3["Peak"] = "P3"
            # Concatenate all peaks into one dataframe
            df_selected_points_peak = pd.concat(
                [auto_peak_1, auto_peak_2, auto_peak_3], ignore_index=True
            )
            df_selected_points_peak["File"] = subject_file

        if subject_file[-2:] == "_1" or subject_file[-2:] == "_4":
            auto_peak_3 = temp_df.loc[
                (temp_df["x"] >= con_1_4_ranges[0])
                & (temp_df["x"] <= con_1_4_ranges[1])
            ]
            auto_peak_3 = auto_peak_3.loc[
                (auto_peak_3.groupby(["Channel"])["y"].idxmax())
            ]
